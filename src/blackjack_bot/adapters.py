...
class NoOpCaptureAdapter(CaptureAdapter):
    ...
    def stream(self) -> Iterator[PipelineEvent]:
        ...
        config_commands = {
            "zonesConfig": "configureZones",
            "rules": "configureRules",
            "countProfile": "configureCountProfile",
            "riskModel": "configureRiskModel",
        }
        for key, command_name in config_commands.items():
            if key in payload:
                yield CommandEvent(
                    timestamp=0.0,
        ...
        has_begin_round = any(
            isinstance(event, dict) and event.get("command") == "beginRound"
            for event in payload.get("events", [])
        )
        if not has_begin_round:
            yield CommandEvent(timestamp=0.0, command="beginRound", payload={})
...
class InMemoryPersistenceAdapter(PersistenceAdapter):
    ...
    def save_round(
        ...
        for event in event_list:
            if isinstance(event, CommandEvent) and event.command == "roundSummary":
                summary_payload = event.payload
                break

        if summary_payload is not None:
            export = dict(summary_payload)
            export.update(
                {
